@defcomp GDP begin
    ...
    ge_empirical_distribution_lowpass = Parameter(index=[draw], unit="none")
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_first(t)
            ...
            if p.ge_use_empiricaldistribution == 1.
                ...
                p.ge_growtheffects = p.ge_empirical_distribution_lowpass[Random.rand(1:5000)]
            elseif p.ge_use_empiricaldistribution == 2. # set negative rho values to zero if switch set to two
                Random.seed!(trunc(Int, p.ge_seed_empiricaldistribution))
                p.ge_growtheffects = max(p.ge_empirical_distribution_lowpass[Random.rand(1:5000)], 0)
            end
        end
