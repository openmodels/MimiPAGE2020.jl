    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.region
            ...
            if is_first(t)
                ...
                if isequal(r, 8)
                    v.rgdp_per_cap_NonMarketRemainGDP_sum = sum(p.rgdp_per_cap_NonMarketRemainGDP[t,:]) * p.yagg_periodspan[t]
                end
            else
                ...
                if isequal(r, 8)
                    v.rgdp_per_cap_NonMarketRemainGDP_sum = v.rgdp_per_cap_NonMarketRemainGDP_sum + sum(p.rgdp_per_cap_NonMarketRemainGDP[t,:]) * p.yagg_periodspan[t]
                end
            end
        end
    end
...
function addnonmarketdamages_annual(model::Model)
    nonmarketdamagescomp = add_comp!(model, NonMarketDamages_annual)
