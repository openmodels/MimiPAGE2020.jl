...
@defcomp TotalAbatementCosts begin
    ...
    function run_timestep(p, v, d, t)
        tct_totalcosts_partial_region = p.tc_totalcosts_n2o[t, :] .+ p.tc_totalcosts_ch4[t, :] .+ p.tc_totalcosts_linear[t, :]
        tct_percap_totalcostspercap_partial_region = tct_totalcosts_partial_region ./ p.pop_population_region[t, :]

        tct_percap_totalcostspercap_partial = regiontocountry(p.model, tct_percap_totalcostspercap_partial_region)

        for cc in d.country
            v.tct_totalcosts[t, cc] = p.tc_totalcosts_co2[t, cc] + tct_percap_totalcostspercap_partial[cc] * p.pop_population[t, cc]
            v.tct_percap_totalcostspercap[t, cc] = v.tct_totalcosts[t, cc] / p.pop_population[t, cc]
        end
