@defcomp PAGE09Discontinuity begin
    ...
    distau_discontinuityexponent_2009 = quantile(TriangularDist(20, 200, 50), cdf(TriangularDist(10, 30, 20), p.distau_discontinuityexponent))
    tdis_tolerabilitydisc_2009 = quantile(TriangularDist(2, 4, 3), cdf(TriangularDist(1, 2, 1.5), p.tdis_tolerabilitydisc))

    v.idis_lossfromdisc[t] = max(0, p.rt_g_globaltemperature[t] - tdis_tolerabilitydisc_2009)
    ...
    if is_first(t)
        ...
        v.expfdis_discdecay[t] = exp(-(p.y_year[t] - p.y_year_0) / distau_discontinuityexponent_2009)
    else
        if v.idis_lossfromdisc[t] * (p.pdis_probability / 100) > p.rand_discontinuity
            v.occurdis_occurrencedummy[t] = 1
        ...
        v.expfdis_discdecay[t] = exp(-(p.y_year[t] - p.y_year[t - 1]) / distau_discontinuityexponent_2009)
    end

    for r in d.region
        v.irefeqdis_eqdiscimpact[r] = p.wincf_weightsfactor[r] * wdis_gdplostdisc_2009
