...
for jj_scen in ["NDCs", "2.5 degC Target",  "NDCs Partial", "BAU", "RCP2.6 & SSP1", "RCP4.5 & SSP2", "RCP8.5 & SSP5"]
    ...
    writedlm(string(dir_output, "gdp_scen", jj_scen, "_ge0.0.csv"), hcat(["year"; myyears], [permutedims(myregions); m[:GDP, :gdp]]), ",")
...
df_ge = DataFrame(modelspec = "-999", ge = -999., boundperc = -999., scen = "-999", equiw = -999., eqwshare = -999., gdploss = -999., disc = -999.,
                        te = -999.,
                        tac = -999.,
                        tmc = -999.,
                        timp = -999.,
...
for jj_spec in ["Region",  "Burke", "RegionBayes", "PAGE09"] # RegionBayes default
    for jj_ge in [0:0.05:1;] # 0. default
        for jj_bound in [1:1:10;] # 5 default
            for jj_scen in ["NDCs", "2.5 degC Target",  "NDCs Partial", "BAU", "RCP2.6 & SSP1", "RCP4.5 & SSP2", "RCP8.5 & SSP5"]
                # print out the current state
                print(string("Scenario: ", jj_scen, " Spec: ", jj_spec, " GE: ", jj_ge, " Bound:", jj_bound))

                if jj_scen != "NDCs" && (jj_bound != 1 || jj_spec != "RegionBayes")
                    continue
                end
                ...
                update_param!(m, :cbshare_pcconsumptionboundshare, jj_bound)

                run(m)

                writedlm(string(dir_output, "gdp_scen", jj_scen, "_ge", jj_ge, "_modelspec", jj_spec, "_permafrYes_bound", jj_bound, ".csv"),
                            hcat(["year"; myyears], [permutedims(myregions); m[:GDP, :gdp]]), ",")
                writedlm(string(dir_output, "percapitagdp_scen", jj_scen, "_ge",  jj_ge, "_modelspec", jj_spec, "_permafrYes_bound", jj_bound, ".csv"),
                            hcat(["year"; myyears], [permutedims(myregions); m[:GDP, :gdp] ./ m[:Population, :pop_population]]), ",")
                writedlm(string(dir_output, "grw_gdpgrowthrate_scen", jj_scen, "_ge", jj_ge, "_modelspec", jj_spec, "_permafrYes_bound", jj_bound, ".csv"),
                    ...
                    for jj_eqwshare in [0.95, 0.99, 0.999] # 0.99 default
                        for jj_disc in [0., 3., 4.25, 7.] # 0. default
                            for jj_gdploss in [0., 1.] # 0. default
                                ...
                                if jj_equiw == 1. && jj_disc != 0.
                                    continue
                                elseif jj_spec != "RegionBayes" && (jj_bound != 1 || jj_eqwshare != 0.99)
                                    continue
                                elseif (jj_equiw == 0. || jj_gdploss == 0.) && jj_eqwshare != 0.99
                                    continue
                                elseif jj_ge == 0. && (jj_bound != 1 || jj_eqwshare != 0.99 || jj_gdploss != 0.)
                                    continue
                                elseif (jj_spec in ["Region", "Burke"]) && jj_ge != 0.
                                    continue
                                elseif jj_spec == "PAGE09" && (jj_bound != 1 || jj_eqwshare != 0.99 || jj_disc != 0.)
                                    continue
                                elseif (jj_bound != 5 && jj_bound != 1) && (jj_eqwshare != 0.99 || jj_disc != 0.)
                                    continue
                                elseif jj_scen != "NDCs" && (jj_bound != 1 || jj_ge != 0. || jj_equiw != 1. || jj_disc != 0.)
                                    continue
                                end


                                update_param!(m, :equity_proportion, jj_equiw)
                                update_param!(m, :eqwshare_shareofweighteddamages, jj_eqwshare)
                                update_param!(m, :discfix_fixediscountrate, jj_disc)
                                update_param!(m, :lossinc_includegdplosses, jj_gdploss)

                                run(m)

                                global scc_object = compute_scc_mm(m, year = 2020)

                                push!(df_ge, [jj_spec, jj_ge, jj_bound, jj_scen, jj_equiw , jj_eqwshare, jj_gdploss, jj_disc,
...
df_ge = df_ge[df_ge[:modelspec] .!= "-999", :]
            ...
            for jj_permafr in ["Yes"]
                for jj_gdploss in [0., 1.]
                    for jj_civvalue in [1., 1000000.]
                        for jj_bound in [5., 7.5, 10., 2.5, 1]
                        ...
                        if  jj_gdploss == 0. && jj_civvalue != 1.
                            continue
                        elseif jj_eqwshare != 0.99 && (jj_gestring != "MILD" || jj_bound != 5. || jj_gdploss != 1. || jj_civvalue == 1.)
                            continue
                        elseif jj_bound != 5. && (jj_gestring != "MILD" || jj_eqwshare != 0.99 || jj_gdploss != 1. || jj_civvalue == 1.)
                            continue
                        end
                        ...
                        dir_MCoutput = string(dir_output, "montecarlo/ge", jj_gestring, "_spec", jj_spec,
                                                        ...
                                                        "_civ", jj_civvalue, "_bound", jj_bound,
