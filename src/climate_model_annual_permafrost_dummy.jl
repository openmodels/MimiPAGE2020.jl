...
function climatemodel(scenario::String, use_permafrost::Bool=true, use_seaice::Bool=false)
    ...
    if use_permafrost
        ...
        permafrost_sibcasa_ann = add_comp!(m, PermafrostSiBCASA_annual)
        permafrost_jules = add_comp!(m, PermafrostJULES)
        permafrost_jules_ann = add_comp!(m, PermafrostJULES_annual)
        permafrost = add_comp!(m, PermafrostTotal)
        permafrost_ann = add_comp!(m, PermafrostTotal_annual)
    end
    co2emit = add_comp!(m,co2emissions)
    co2cycle = addco2cycle(m, use_permafrost)
    # TODO: add annual co2 cycle for permafrost
    add_comp!(m, co2forcing)
    ch4emit = add_comp!(m, ch4emissions)
    ch4cycle = addch4cycle(m, use_permafrost)
    # TODO: add annual ch4 cycle for permafrost
    ...
    # TODO: add annual total forcing for permafrost
    ...
    if use_permafrost
        ...
        permafrost_sibcasa_ann[:rt_g_ann] = climtemp_ann[:rt_g_globaltemperature_ann]
        permafrost_jules[:rt_g] = climtemp[:rt_g_globaltemperature]
        permafrost_jules_ann[:rt_g_ann] = climtemp_ann[:rt_g_globaltemperature_ann]
        permafrost[:perm_sib_ce_co2] = permafrost_sibcasa[:perm_sib_ce_co2]
        permafrost_ann[:perm_sib_ce_co2_ann] = permafrost_sibcasa_ann[:perm_sib_ce_co2_ann]
        permafrost[:perm_sib_e_co2] = permafrost_sibcasa[:perm_sib_e_co2]
        permafrost_ann[:perm_sib_e_co2_ann] = permafrost_sibcasa_ann[:perm_sib_e_co2_ann]
        permafrost[:perm_sib_ce_ch4] = permafrost_sibcasa[:perm_sib_ce_ch4]
        permafrost_ann[:perm_sib_ce_ch4_ann] = permafrost_sibcasa_ann[:perm_sib_ce_ch4_ann]
        permafrost[:perm_jul_ce_co2] = permafrost_jules[:perm_jul_ce_co2]
        permafrost_ann[:perm_jul_ce_co2_ann] = permafrost_jules_ann[:perm_jul_ce_co2_ann]
        permafrost[:perm_jul_e_co2] = permafrost_jules[:perm_jul_e_co2]
        permafrost_ann[:perm_jul_e_co2_ann] = permafrost_jules_ann[:perm_jul_e_co2_ann]
        permafrost[:perm_jul_ce_ch4] = permafrost_jules[:perm_jul_ce_ch4]
        permafrost_ann[:perm_jul_ce_ch4_ann] = permafrost_jules_ann[:perm_jul_ce_ch4_ann]
    end
    ...
    # TODO: add annualise CO2 cycle / total forcing
    connect_param!(m, :TotalForcing => :f_CH4forcing, :ch4forcing => :f_CH4forcing)
    # TODO: add annualise CH4 cycle / total forcing
