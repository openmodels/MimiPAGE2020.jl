...
Here is an example of computing the social cost of carbon with
PAGE-2020. Note that the units of the returned value are dollars $/ton
CO2.

The example below is written for when PAGE-2020 is installed as a
package. The corresponding code for computing the SCC when installed
as a repository is to `include("src/compute_scc.jl")` and then to run
the functions below without the `MimiPAGE2020.` prefix.
...
```
The first argument to the `compute_scc` function is a PAGE-2020 model, and it is an optional argument. If no model is provided, the default PAGE-2020 model will be used. 
